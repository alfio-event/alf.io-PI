...
class LabelManagerTest {
    ...
    fun testGenerateLabelWitLayoutNull() {
        ...
        assertEquals("${ticket1.uuid.substringBefore("-")}::${ticket1.lastName}::${ticket1.firstName}::${ticket1.additionalInfo!!["company"].orEmpty().take(50)}::${ticket1.email}", result.qrContent)
        ...
        assertEquals("${ticket2.uuid.substringBefore("-")}::${ticket2.lastName}::${ticket2.firstName}::${ticket2.additionalInfo?.get("company").orEmpty()}::${ticket2.email}", result2.qrContent)
